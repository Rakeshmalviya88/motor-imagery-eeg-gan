...
# ignoring some annoying mne warnings about channel types
...
latent_dim = 100
gp_weight = 10.0 # standard gp weight from the wgan-gp paper
n_critic = 5
batch_size = 64
...
def load_and_preprocess_subject(subject, run_type="T"):
    ...
    # if it's the eval file, we need to fix the missing labels using the .mat file
    if run_type == "E":
        mat_path = file_path.replace(".gdf", ".mat")
        mat_data = scipy.io.loadmat(mat_path)
        classlabel = mat_data['classlabel'].flatten()
        
        # map classes 1-4 to their corresponding event codes
        ...
        for i, desc in enumerate(descriptions):
            if desc == '783': # 783 means unknown label in the dataset
                descriptions[i] = test_annotations[idx]
                idx += 1
        raw.annotations.description = descriptions

    # making sure eog channels are set correctly so they don't get mixed up with eeg
    eog_channels = ["EOG-left", "EOG-central", "EOG-right"]
    present_eog = {ch: "eog" for ch in eog_channels if ch in raw.ch_names}
    if present_eog:
        raw.set_channel_types(present_eog)

    # map weird gdf channel names to standard 10-20 system
    gdf_to_standard = {
    ...
    # using these symmetric motor cortex channels specifically for left/right hand MI!
    target_channels = ['EEG-FC3', 'EEG-FC1', 'EEG-C3', 'EEG-CP3', 'EEG-CP1', 'EEG-FC4', 'EEG-FC2', 'EEG-C4', 'EEG-CP4', 'EEG-CP2']
    raw.pick(target_channels)
    
    # 50 Hz notch to remove european powerline noise
    raw.notch_filter(50, verbose="ERROR")
    # bandpass 8-30 Hz to isolate Mu and Beta bands where motor imagery happens
    ...
    # epoching from 2 to 6 seconds (the motor imagery task window)
    ...
    # map events to 0 and 1 for the neural network
    subject_label_map = {}
    ...
    # min max scaling to -1, 1 for the GAN
...
def z_score_normalize_subject(X_train_data, X_test_data):
    # z-score normalization to make mean 0 and std 1
    ...
    X_train_data = np.nan_to_num(X_train_data) # catching any nan values just in case
...
def resize_1d(x, new_length):
    # helper for upsampling eeg signals in the generator
...
def make_generator(n_channels=10):
    ...
    # embedding the label so the gan can generate specific classes (left or right)
    ...
    # gradually upsampling to get to 1001 timepoints
...
def make_critic(n_channels=10):
    ...
    # embedding for the critic to judge if the class matches the eeg
    label_emb = Embedding(input_dim=2, output_dim=50)(label_in)
    ...
    x = Flatten()(x)
    x_out = Dense(1)(x) # linear output for WGAN (no sigmoid!)
...
def train_wgan_gp_for_subject(X_real_data, y_train_data, epochs=100, verbose=False, n_channels=10):
    ...
    # converting to channels_last for tensorflow
    ...
    def train_step(real_eeg, labels):
        # train critic n_critic times for every 1 generator step
...
def train_cnn_classifier(X_train_data, y_train_data, epochs=150, verbose=0):
    # balancing class weights just in case augmentation makes it slightly uneven
...
def train_and_evaluate_subject(subject_id, epochs_wgan=150, epochs_cnn=150):
    ...
    # Generate a massive pool of synthetic data (3x the train size) so the critic can evaluate them
    pool_multiplier = 3
    ...
    # critic grades all the synthetic samples
    scores = critic_subj.predict([synthetic_raw_pool, y_fake_pool[:, np.newaxis]], verbose=0).flatten()
    
    # selecting only the absolute best scoring samples to add 50% overall augmentation
    target_num_per_class = int(0.25 * len(X_train_subj))
    selected_indices = []
    for cls in [0, 1]:
        cls_candidate_idx = np.where(y_fake_pool == cls)[0]
        cls_scores = scores[cls_candidate_idx]
        
        # sort scores highest to lowest and skim the top
    ...
    # run cnn multiple times and keep the best one to avoid bad random initialization
    for run in range(6):
