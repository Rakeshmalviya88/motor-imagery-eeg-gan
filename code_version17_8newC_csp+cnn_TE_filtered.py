...
def load_and_preprocess_subject(subject, run_type="T"):
    ...
    # SYMMETRIC MOTOR CORTEX CHANNELS FOR LEFT/RIGHT HAND MI
    target_channels = ['EEG-FC3', 'EEG-FC1', 'EEG-C3', 'EEG-CP3', 'EEG-FC4', 'EEG-FC2', 'EEG-C4', 'EEG-CP4']
...
def make_generator(n_channels=8):
...
def train_wgan_gp_for_subject(X_real_data, y_train_data, epochs=100, verbose=False, n_channels=8):
...
def train_and_evaluate_subject(subject_id, epochs_wgan=150, epochs_cnn=150):
    print(f"Loading T and E files for Subject A{subject_id:02d}...")
    ...
    generator_subj, critic_subj = train_wgan_gp_for_subject(X_train_subj, y_train_subj, epochs=epochs_wgan, verbose=False, n_channels=8)
    
    # Generate a massive pool of synthetic data (3x the train size) to filter the absolute best
    pool_multiplier = 3
    ...
    # Select top 25% per class to do 50% augmentation
