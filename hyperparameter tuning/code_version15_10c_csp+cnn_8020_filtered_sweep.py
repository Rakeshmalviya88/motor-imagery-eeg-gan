...
def make_generator(latent_dim, n_channels=10):
...
def train_wgan_gp_for_subject(X_real_data, y_train_data, latent_dim, n_critic, batch_size, gen_lr, epochs=100, verbose=False, n_channels=10):
    X_real_data = X_real_data.astype(np.float32)
    X_real_gan = np.transpose(X_real_data[..., 0], (0, 2, 1))
    
    generator = make_generator(latent_dim=latent_dim, n_channels=n_channels)
    critic = make_critic(n_channels=n_channels)

    # Note: TTUR - Critic LR is kept at 1e-4, while Gen LR is swept
...
# --- SWEEP EVALUATION PIPELINE ---
def train_and_evaluate_subject(subject_id, X_full_norm, y_full_subj, params, epochs_wgan=250, epochs_cnn=150):
    ...
    generator_subj, critic_subj = train_wgan_gp_for_subject(
    ...
    noise_pool = np.random.normal(size=(pool_size, params["latent_dim"])).astype(np.float32)
    ...
    # 4 CNN Runs as requested (Speed Optimization)
    for run in range(4):
...
if __name__ == "__main__":
    ...
    print("\n" + "="*79)
    print(f"STARTING GAN HYPERPARAMETER SWEEP ({len(combinations)} COMBINATIONS)")
    print("="*79)

    print("\nPre-loading and filtering all MNE data into RAM to bypass repetitive I/O overhead...")
    preloaded_data = {}
    for subject in range(1, 10):
        print(f"   -> Loading Subject A{subject:02d}...")
        preloaded_data[subject] = load_and_preprocess_subject(subject, "T")
    print("Done pre-loading. Starting the sweep loop!\n")

    sweep_results = []
    completed_configs = []
    
    if os.path.exists("gan_sweep_results.csv"):
        print("Found existing gan_sweep_results.csv! Loading previous progress to resume...")
        existing_df = pd.read_csv("gan_sweep_results.csv")
        sweep_results = existing_df.to_dict('records')
        
        for res in sweep_results:
            config = {
                "n_critic": int(res["n_critic"]),
                "batch_size": int(res["batch_size"]),
                "gen_lr": float(res["gen_lr"]),
                "latent_dim": int(res["latent_dim"])
            }
            completed_configs.append(config)

    for idx, params in enumerate(combinations):
        if params in completed_configs:
            print(f"\n[SWEEP {idx+1}/{len(combinations)}] Skipping already completed config: {params}")
            continue
            
        print(f"\n[SWEEP {idx+1}/{len(combinations)}] Testing Params: {params}")
        
        accs = []
        for subject in range(1, 10):
            X_full_norm, y_full_subj = preloaded_data[subject]
            # Evaluate using 250 GAN epochs and 150 CNN epochs
            acc, prec, rec, f1 = train_and_evaluate_subject(subject, X_full_norm, y_full_subj, params, epochs_wgan=250, epochs_cnn=epochs_cnn_value)
            accs.append(acc)
            print(f"   Sub A{subject:02d} -> Acc: {acc:.4f}")
            
        avg_acc = np.mean(accs)
        print(f"-> Sweep {idx+1} Result | Avg Accuracy: {avg_acc:.4f}")
        ...
        # Save to CSV after every combination in case the notebook crashes
        df = pd.DataFrame(sweep_results)
        df.to_csv("gan_sweep_results.csv", index=False)
        
    print("\n" + "="*79)
    print("SWEEP COMPLETE. SORTED RESULTS:")
    print("="*79)
    final_df = pd.DataFrame(sweep_results).sort_values(by="avg_accuracy", ascending=False)
