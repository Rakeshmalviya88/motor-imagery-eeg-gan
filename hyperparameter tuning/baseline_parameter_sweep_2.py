...
# Constants
csv_path = f"baseline_sweep_results_2_part{args.part}.csv"
...
def main():
    ...
    csp_components = [4, 6]          # n_components for CSP
    depth_multipliers = [1, 2]       # DepthwiseConv2D depth multiplier
    learning_rates = [1e-3, 5e-4]    # learning rate for Adam
    batch_sizes = [16, 32]           # batch size for training
    ...
    print(f"\n========== STARTING Baseline Parameter Sweep 2 (Partition {args.part}/2 | Configs {start_idx}-{start_idx+len(configs_to_run)-1}) ==========")
    sys.stdout.flush()
    ...
    for offset, config in enumerate(configs_to_run):
        ...
        for subj in range(1, 10):
            ...
            X_train_norm, y_train = epoch_and_scale(raw_trains[subj], TMIN, TMAX)
            X_test_norm, y_test = epoch_and_scale(raw_tests[subj], TMIN, TMAX)
    ...
    print("\n========== BASELINE PARAMETER SWEEP 2 COMPLETED SUCCESSFULLY! ==========")
    sys.stdout.flush()
