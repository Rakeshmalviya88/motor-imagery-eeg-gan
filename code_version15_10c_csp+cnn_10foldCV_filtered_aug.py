...
# --- AUGMENTED PIPELINE WITH 10-FOLD CV ---
def train_and_evaluate_subject_cv(subject_id, epochs_wgan=150, epochs_cnn=150):
    ...
    fold = 1
    for train_index, test_index in skf.split(X_full_norm, y_full_subj):
        print(f"\n--- Subject A{subject_id:02d} | Fold {fold}/10 (Augmented) ---")
        X_train_norm, X_test_norm = X_full_norm[train_index], X_full_norm[test_index]
        y_train_subj, y_test_subj = y_full_subj[train_index], y_full_subj[test_index]
        ...
        # 100% augmentation total (50% per class based on train size)
        ...
        print(f"   -> Fold {fold} Best CNN Accuracy: {best_acc:.4f}")
        fold += 1
            
    return np.mean(fold_accs), np.mean(fold_precs), np.mean(fold_recs), np.mean(fold_f1s)


if __name__ == "__main__":
    print("\n" + "="*79)
    print("STARTING INDIVIDUAL SUBJECT TRAINING AND EVALUATION (AUGMENTED 10-FOLD CV)")
    print("="*79)

    subject_results = {}
    for subject in range(7, 10):
        print(f"\n[SUBJECT A{subject:02d} - AUGMENTED]")
        acc, prec, rec, f1 = train_and_evaluate_subject_cv(subject, epochs_wgan=epochs_wgan_value, epochs_cnn=epochs_cnn_value)
        subject_results[subject] = {"accuracy": acc, "precision": prec, "recall": rec, "f1": f1}
        print(f"Subject A{subject:02d} Results (10-Fold Avg) - Accuracy: {acc:.4f}, F1: {f1:.4f}")

    print("\n" + "="*79)
    print("INDIVIDUAL SUBJECT-WISE EVALUATION SUMMARY (AUGMENTED 10-FOLD CV)")
    print("="*79)
    ...
    print("\n" + "="*79)
    print("STARTING BASELINE EVALUATION (REAL DATA ONLY - 10-FOLD CV)")
    print("="*79)
    ...
    for subject in range(1, 10):
        ...
        fold = 1
        for train_index, test_index in skf.split(X_full_norm, y_full_subj):
            print(f"\n--- Subject A{subject:02d} | Fold {fold}/10 (Baseline) ---")
            X_train_norm, X_test_norm = X_full_norm[train_index], X_full_norm[test_index]
            y_train_subj, y_test_subj = y_full_subj[train_index], y_full_subj[test_index]
            ...
            fold_accs_b.append(best_acc)
            fold_precs_b.append(best_prec)
            fold_recs_b.append(best_rec)
            fold_f1s_b.append(best_f1)
            print(f"   -> Fold {fold} Best CNN Accuracy: {best_acc:.4f}")
            fold += 1
            
        subj_acc = np.mean(fold_accs_b)
        subj_prec = np.mean(fold_precs_b)
        subj_rec = np.mean(fold_recs_b)
        subj_f1 = np.mean(fold_f1s_b)
        
        baseline_results[subject] = {"accuracy": subj_acc, "precision": subj_prec, "recall": subj_rec, "f1": subj_f1}
        print(f"Subject A{subject:02d} Baseline (10-Fold Avg) - Accuracy: {subj_acc:.4f}, F1: {subj_f1:.4f}")

    print("\n" + "="*79)
    print("BASELINE EVALUATION SUMMARY (REAL DATA ONLY - 10-FOLD CV)")
    print("="*79)
